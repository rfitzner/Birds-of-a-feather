 ...
 */
public class SimulationSetOneHeavyTail {
	double[] ps,xi,mus;
	double rho;
	...
	public SimulationSetOneHeavyTail(int nr, double p1, double xi1, double xi2, double mu1, double mu2, int nrSimulations) {
		super();
		...
		if ((this.mus[0]<=0)|| (this.rho<=1)) valideParameters=false;
		
		if (this.ps[0]==0) {
			if ((this.mus[0]!=2) || (this.xi[0]!=1)) valideParameters=false;
		}
		if (this.ps[0]==1) {
			if ((this.mus[1]!=2) || (this.xi[1]!=1) ) valideParameters=false;
		}
		this.nrNodes = nr;
	...
	public void performSimulation(){
		if(valideParameters){
			...
			for(int i =0; i<this.nrSimulations;i++){
				OneGraphOneHeavyTail model= new OneGraphOneHeavyTail(nrNodes, ps[0],xi[0],xi[1],mus[0],mus[1]);
				model.labeling();
